        ...
        <form action="{{ route('projects.tasks.update', [$project, $task]) }}" method="POST">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-2.5">{{ old('description', $task->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" value="{{ old('due_date', $task->due_date?->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Estimated Hours</label>
                    <input type="number" name="estimated_hours" value="{{ old('estimated_hours', $task->estimated_hours) }}" step="0.5" min="0" class="w-full border border-gray-300 rounded-lg px-4 py-2.5">
                </div>
            </div>
            <div class="flex justify-between mt-6 pt-6 border-t border-gray-100">
                <a href="{{ route('projects.tasks.show', [$project, $task]) }}" class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg text-sm">Cancel</a>
