                ...
                <nav class="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
                    ...
                    @if (auth()->user()->isAdmin() || auth()->user()->isSuperAdmin())
                        <a href="{{ route('admin.operators.index') }}"
                            class="sidebar-item flex items-center px-4 py-3 rounded-lg transition-all {{ request()->routeIs('admin/operators.*') ? 'active' : '' }}">
                            <i class="fas fa-folder-open w-5 mr-3"></i> Operators
                        </a>
                    @endif
                    ...
                    @if (auth()->user()->isAdmin() || auth()->user()->isSuperAdmin())
                        ...
                        @if (auth()->user()->isSuperAdmin())
                            <a href="{{ route('admin.roles.index') }}"
                                class="sidebar-item flex items-center px-4 py-3 rounded-lg transition-all {{ request()->routeIs('admin.roles.*') ? 'active' : '' }}">
